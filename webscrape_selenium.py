...
city_data	= pd.read_csv("geo_test.csv")
...
headers="Data ID, Name, City, Country, Venue Type, Option, Rating, Reviews, Price, Latitude, Longitude, Phone, URL \n"
data = list()
data.append(headers)
...
for i in range(len(city_names)):
	...
	my_url = "https://www.happycow.net/searchmap?location=&radius=25&metric=mi&limit=81&order=default&lat="+str(city_lat[i])+"&lng="+str(city_lng[i])
	driver.get(my_url)
	...
	for count in range(1, (loop_count+1)):
		...
		if (count != 1):
			...
			my_url="https://www.happycow.net/searchmap?location=&radius=25&metric=mi&limit=81&order=default&lat="+str(city_lat[i])+"&lng="+str(city_lng[i])+"&page="+str(count)
			driver.get(my_url)
			page_html=driver.page_source
			soup_page = soup(page_html,'html.parser')
